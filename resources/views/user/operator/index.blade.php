...
@section('content')
    ...
    <div class="card mt-3">
        <div class="card-header">
            <x-button route="web.user.operator.create" size="sm" color="primary" class="text-white">Tambah Operator
                Baru</x-button>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td>
                                    @if ($user->id != 2)
                                        <x-button route="web.user.operator.edit" :parameter="$user" size="sm"
                                            color="warning" class="text-white">Edit</x-button>
                                        <form action="{{ route('web.user.operator.destroy', $user) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            @method('delete')
